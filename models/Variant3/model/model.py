...
import copy
import yaml
from models.Variant3.model.CCGNode import CCGNode
from models.Variant3.scripts.preprocess import create_mapping

def load_config():
    """The function loads the config from the config.yaml file"""
    with open('models/Variant3/config/config.yaml', 'r') as file:
...
class Decoder(nn.Module):
    ...
    def __init__(self, tgt_vocab, enc_to_idx, atomic_to_idx):
        ...
        # Embedding layer for the other leaves in the context window
        self.other_leaves_attention = nn.Sequential(
            nn.Embedding(len(atomic_to_idx), self.config['model']['d_model']),
        )
        # Linear layer to process the other leaves in the context window
        self.attention_layer = nn.Linear(self.config['model']['d_model'], 1, bias=False).to(self.config['model']['device'])
    ...
    def predict_data(self, node, memory, current_depth, other_leaves = None):
        ...
        if other_leaves is None: # root prediction, no other leaves are present
            ...
            # trying to scale the memory by 1/(current_depth+1) so that leaves have more weight as we go deeper in the tree
            out = out + memory * (1/(current_depth+1)) + other_leaves 
    ...
    def grab_other_leaves(self, other_trees, depth, type_of_children = 'both'):
        """ The function grabs the leaves from the other trees at a given depth. """
        all_leaves = []
        with torch.no_grad():
            # Variant3 discriminates between left and right children
            # iterate over the other trees
            for tree in other_trees:
                # if we are constructing a right branch, get the left leaves, if we are constructing a left branch, get the right leaves, otherwise get all leaves
                if type_of_children == 'left':
                    list_of_leaves = tree.get_leaves_left(depth-1)
                elif type_of_children == 'right':
                    list_of_leaves = tree.get_leaves_right(depth-1)
                else:
                    list_of_leaves = tree.get_leaves(depth-1)
                # add the leaves to the list
        ...
        if len(leaves_features) == 0: # weird cases in which no leaf is present in the other trees, then return None and predict label using only the memory
            return None
        # embed the leaves
        embeddings = self.other_leaves_attention(leaves_features)
        # calculate the weights
        scores = self.attention_layer(embeddings)
        # normalize the weights
        scores = F.softmax(scores, dim = 0)
        # get the context (weighted sum of the leaves)
        context = torch.sum(scores * embeddings, dim=0)
        return context
    ...
    def build_tree_parallel(self, roots, memories, tgts, mode, current_depth):
        ...
        Variant3 discriminates between left and right children. When predicting nodes and collecting leaves for the context window, it differentiates between left and right children.
        """
        # initialize the logits for the nodes at depth d
        logits = torch.zeros((len(roots), 2**current_depth, self.tgt_vocab)).to(self.config['model']['device'])
        seq_length = len(roots)
        # if we are at the root nodes
        if current_depth == 0:
            for i, root in enumerate(roots):
                # get the BERT encoding for the word
                memory = memories[i]
        ...
        # depth >= 1
            ...
            for i, root in enumerate(roots):
                ...
                with torch.no_grad():
                    nodes_at_d = root.get_nodes_at_d(current_depth) # this is a list of nodes at depth d
                    ...
                    if all(element is None for element in nodes_at_d):
                        continue
                # get the memory for the root
                memory = memories[i]
                # iterate over the nodes at depth d
                for j, node in enumerate(nodes_at_d):
                    if node is None:
                        continue
                    # get the indices of the other leaves in the context window
                    left_i = max(0, i-self.config['model']['LEAVES_CONTEXT'])
                    right_i = min(seq_length, i+self.config['model']['LEAVES_CONTEXT'])
                    # if we are predicting the result, we look for leaves everywhere (maybe only right children)
                    if node.parent.left == node:
                        other_roots = roots[left_i:i] + roots[i+1:right_i]
                        other_leaves_encoded = self.grab_other_leaves(other_roots, current_depth, 'right')
                    # if we are predicting the argument and the parent is a forward slash, we look for leaves on the right, but only left children
                    elif node.parent.teacher_data == self.atomic_to_idx['/']:
                        other_roots = roots[i+1:right_i]
                        other_leaves_encoded = self.grab_other_leaves(other_roots, current_depth, 'left')
                    # if we are predicting the argument and the parent is a backward slash, we look for leaves on the left, but only left children
                    elif node.parent.teacher_data == self.atomic_to_idx['\\']:
                        other_roots = roots[left_i:i]
                        other_leaves_encoded = self.grab_other_leaves(other_roots, current_depth, 'left')
