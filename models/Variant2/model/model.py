...
import copy
import yaml
from models.Variant2.model.CCGNode import CCGNode
from models.Variant2.scripts.preprocess import create_mapping
...
class Decoder(nn.Module):
    """ The decoder block that includes an AddrMLP model that also uses the leaves present in the other trees in the context window and weights them. """
    def __init__(self, tgt_vocab, enc_to_idx, atomic_to_idx):
        ...
        # Linear layer to process the other leaves in the context window, mapping them to a single embedding
    ...
    def grab_other_leaves(self, other_trees, depth):
        ...
        if len(leaves_features) == 0:
            return None
        # calculate weights for the other leaves
