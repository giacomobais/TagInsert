...
"""Many classes used for implementing the TagInsertL2R model are taken from the
 Annotated Transformer tutorial by Alexander Rush, with some modifications to fit the needs of the project
...
class EncoderDecoder(nn.Module):
    ...
    other models.
    The encoder and decoder are the same as the one in the VanillaEncoderDecoderTransformer model.
    The only difference is the addition of the words embeddings to the target sequence.
    """
...
class Embeddings(nn.Module):
    """
    Embeddings class for the words
    It is kept empty as we are using the BERT embeddings. We retained the class for future use.
    """
    def __init__(self, d_model, vocab):
...
class POS_Embeddings(nn.Module):
    """
    Embeddings class for the POS tags
    """
    def __init__(self, d_model, vocab):
...
def make_model(
    src_vocab, tgt_vocab, N=6, d_model=768, d_ff=768*4, h=8, dropout=0.1
):
